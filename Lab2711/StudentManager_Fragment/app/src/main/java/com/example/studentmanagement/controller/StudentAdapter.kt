...
class StudentAdapter(private var students: MutableList<StudentModel>, private var listener: OnStudentFragment) :
    RecyclerView.Adapter<StudentAdapter.StudentViewHolder>(), StudentActionListener
{
    ...
    override fun onEditStudent(position: Int, editStudent: StudentModel) {
        val student = students[position]
        students[position] = editStudent
        notifyItemChanged(position)
    }

    override fun onRemoveStudent(position: Int, student: StudentModel, context: Context) {
        students.removeAt(position)
        notifyItemRemoved(position)
...
public interface StudentActionListener{
    fun onEditStudent(position: Int, editStudent: StudentModel)
    fun onRemoveStudent(position: Int, student: StudentModel, context: Context)
